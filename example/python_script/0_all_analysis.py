...
from sovapy.core.analysis import (PDF_Analysis, BondAngle_Analysis, Tetrahedral_Order_Analysis,
                                  Ring_Analysis, Cavity_Analysis)
...
pdf = PDF_Analysis(atoms, dr=0.05, dq=0.05, qmin=0.3, qmax=25.0)
pdf.run()
pdf.plot()
...
bond_angle = BondAngle_Analysis(atoms, bins=50)
bond_angle.run()
bond_angle.plot()
...
tetra = Tetrahedral_Order_Analysis(atoms, bins=100, list_cc_dist=list_cc_dist)
tetra.run()
tetra.plot()
tetra.save_to_hdf5()
print('\n\n')
...
# Ring analysis
print('Ring analysis:')
ring = Ring_Analysis(atoms, guttman=True, king=True, primitive=False, cutoff_primitive=24)
ring.run()
ring.plot()
ring.save_to_hdf5()
print('\n\n')
...
cavity = Cavity_Analysis(atoms, resolution=64, cutoff_radii=cutoff_radii)
cavity.run()
cavity.plot()
cavity.save_to_hdf5()
print('\n\n')
