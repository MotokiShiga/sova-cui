...
from .utils import *
from .spacegroupdata import Hex2RhombHall, Number2Hall, HM2Hall, Hall2Number, Rhomb2HexHall, Hall2HM, SymOpsHall
from .elementdata import ElementData
import time

try:
    from .calculate_atomsdata import calculate_atomsdata_list
    from .calculate_atomsdata import search_removeindices
    has_calculate_atomsdata = True
except ImportError:
    has_calculate_atomsdata = False
    print("Not found 'calculate_atomsdata' library in cif2cell/uctools\n")
...
class CellData(GeometryObject):
    ...
    def __init__(self,verbose=0):
        ...
        self.verbose = verbose # 1 output message
    ...
    def getCrystalStructure(self, reducecell=False):
        ...
        if has_calculate_atomsdata:
            positions = []
            for atom in self.atomdata:
                positions.append((atom[0].position[0], atom[0].position[1], atom[0].position[2]))
    
            removeindices = calculate_atomsdata_list(positions)
        ...
        if has_calculate_atomsdata:
            ...
            length = []
            k = 0
            for i, atom in enumerate(self.atomdata):
                start_idx.append(k)
                length.append(len(atom))
                for j in range(len(atom)):
                    positions.append((atom[j].position[0], atom[j].position[1], atom[j].position[2]))
                    k += 1
                    
            removeindices = search_removeindices(positions, start_idx, length)
        else:            
            removeindices = set([])
            for i in range(len(self.atomdata)):
