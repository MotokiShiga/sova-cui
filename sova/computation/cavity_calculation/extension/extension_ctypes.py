...
lib.subgrid_create.argtypes = [
        c_int,     # cubesize
        c_int * 3] # grid_dimensions
...
def atomstogrid(grid, discrete_positions, radii_indices, discrete_radii, translation_vectors, discretization_grid):
    ...
    strides = (c_int * 3)(*[ int(s / grid.itemsize) for s in grid.strides])
    ...
    discretization_grid_strides = (c_int * 3)(*[int(s / discretization_grid.itemsize) for s in discretization_grid.strides])
...
def subgrid_add_domains(sg, domain_indices, domain_points, translation_vectors):
    ...
    lib.subgrid_add_domains(sg, npoints_c, domain_indices_c, domain_points_c, ntranslations_c, translations_c)


def mark_cavities_c(grid, domain_grid, discretization_grid, sg, use_surface_points):
    dimensions_c = (c_int * 3)(*grid.shape)
    strides_c = (c_int * 3)(*[int(s / grid.itemsize) for s in grid.strides])
    ...
    discgrid_strides_c = (c_int * 3)(*[int(s / discretization_grid.itemsize) for s in discretization_grid.strides])
...
def cavity_triangles(cavity_grid,
    ...
    strides_c = (c_int * 3)(*[int(s / cavity_grid.itemsize) for s in cavity_grid.strides])
