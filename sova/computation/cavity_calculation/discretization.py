...
from .extension.extension_ctypes import mark_translation_vectors
try:
    ...
    def get_equivalent_point_in_volume(self, point):
        ...
        while not all(0 <= p < s for p, s in zip(point, self.grid.shape)):
            if cell_center is None:
                cell_center = np.array(self.grid.shape, dtype=np.int8) / 2
            ...
            vector_to_cell_center = cell_center - np.array(point, dtype=np.int8)
            ...
            rounded_dot_products = np.array(np.round(dot_products), dtype=np.int8)
    ...
    def continuous_to_discrete(self, arg, result_inside_volume=False, unit_exponent=1):
        ...
        def transform_point(point, result_inside_volume):
            if isinstance(point, np.ndarray) and len(point.shape) > 1:
                ...
                result = np.array(np.floor((point + s_tilde_bc / 2) / self.s_step + 0.5), dtype=np.int8)
            ...
            if isinstance(point, np.ndarray) and len(point.shape) > 1:
                if result_inside_volume:
                    if any(isinstance(c, float) for c in point):
                        rounded_point = np.array(np.around(point), dtype=np.int8)
