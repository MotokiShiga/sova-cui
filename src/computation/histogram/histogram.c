...
static PyObject *calc_histogram(PyObject *self, PyObject *args)
{
  ...
  for (i = 0; i < ntypes; i++) 
  {
    ni[i] = PyInt_AsSsize_t(PyList_GetItem(_ni, (Py_ssize_t)i));
  }
...
static PyMethodDef histmethods[] = {
  { "calc_histogram", calc_histogram, METH_VARARGS, "Calculates histogram" },
  { NULL, NULL, 0, NULL }
};


PyMODINIT_FUNC inithistogram()
{
  (void)Py_InitModule("histogram", histmethods);
}
